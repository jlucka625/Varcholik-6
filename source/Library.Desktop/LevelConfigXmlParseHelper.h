...
{
	class LevelConfigXmlParseHelper : public IXmlParseHelper
	{
	public:
		...
		*/
		explicit LevelConfigXmlParseHelper();

		LevelConfigXmlParseHelper(const LevelConfigXmlParseHelper& otherSharedData) = delete;

		LevelConfigXmlParseHelper& operator=(const LevelConfigXmlParseHelper& otherSharedData) = delete;

		/** @brief					Default destructor for the IntegerXmlParseHelper derived class.
		*
		*/
		virtual ~LevelConfigXmlParseHelper() = default;
	...
	private:
		...
		static const std::string LEVEL_TAG;
