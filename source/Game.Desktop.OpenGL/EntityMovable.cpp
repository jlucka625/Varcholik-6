...
EntityMovable::EntityMovable() : mSprite(), mSpriteRenderer(ResourceManager::GetShader("SpriteShader")), mIsPositiveX(true), mIsPositiveY(true)
{
...
void EntityMovable::Update(WorldState & worldState)
{
	...
	//auto lastPosition = mSprite.GetPosition();
	//mSprite.SetPosition(glm::vec2(ConvertMeterToPixel(mBody->GetPosition().x), ConvertMeterToPixel(mBody->GetPosition().y)));
	...
	for (std::uint32_t i = 0; i < entities.Size(); ++i)
	{
		...
		if (tile && tile->Name() == "Barrier")
		{
			if (mSprite.IsColliding(tile->GetSprite()))
			{
				...
				break;
				//mSprite.SetPosition(lastPosition);
...
void EntityMovable::CreateSprite(const std::string & textureName, float u, float v, float width, float height)
{
	...
	mSprite.SetPosition(glm::vec2(u, v));
