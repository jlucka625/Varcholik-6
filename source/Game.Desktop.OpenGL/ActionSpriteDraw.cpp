...
void ActionSpriteDraw::Update(WorldState & worldState)
{
	...
	{
		reinterpret_cast<EntityBackground*>(worldState.mEntity)->Draw();
	}
	else if (worldState.mEntity->Is(EntityText::TypeIdClass()))
	{
		reinterpret_cast<EntityText*>(worldState.mEntity)->Draw();
	}
}
